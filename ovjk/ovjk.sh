...
#YHML
echo -e "\033[31m正在部署环境...以及更新yum\033[0m"
sleep 1
...
rpm -ivh ${http}${host}${ServerLocation}epel-release-6-8.noarch.rpm --force >/dev/null 2>&1
rpm --import /etc/pki/rpm-gpg/RPM-GPG-KEY-EPEL-6
rpm -ivh ${http}${host}${ServerLocation}remi-release-6.rpm --force >/dev/null 2>&1
...
cd /bin
rm -rf xmcx
wget ${http}${host}${ServerLocation}xmcx >/dev/null 2>&1
chmod 777 xmcx

sleep 1
cd /etc
rm -f sysctl.conf
wget ${http}${host}${ServerLocation}sysctl.conf >/dev/null 2>&1
chmod 777 /etc/sysctl.conf
sysctl -p >/dev/null 2>&1

cd /etc/squid
rm -f squid.conf squid_passwd
wget ${http}${host}${ServerLocation}squid.conf >/dev/null 2>&1
wget ${http}${host}${ServerLocation}squid_passwd >/dev/null 2>&1
...
wget ${http}${host}${ServerLocation}peizhi.cfg >/dev/null 2>&1
...
rm -rf www
rm -rf html
wget ${http}${host}${ServerLocation}WEB-YHML.tar.gz >/dev/null 2>&1
sleep 1
...
cd /var/www/html/res/
rm -f jiankong
wget ${http}${host}${ServerLocation}jiankong >/dev/null 2>&1
...
cd /bin
rm -f YHML
wget ${http}${host}${ServerLocation}vpn1 >/dev/null 2>&1
mv vpn1 /bin/YHML
chmod 0777 /bin/YHML
...
http-proxy-option EXT1 Host 114.255.201.163' >ovpn.1
