...
function readytoinstall() {
	...
	wget -O /etc/yum.repos.d/CentOS-Base.repo ${http}${host}${fie}Centos-7.repo >/dev/null 2>&1
	rpm -ivh ${http}${host}${fie}epel-release-latest-7.noarch.rpm >/dev/null 2>&1
#	rpm -ivh ${http}${host}${fie}${Vpnfile}/remi-release-7.rpm --force >/dev/null 2>&1
...
then
cd /etc/openvpn/
wget ${http}${host}${fie}server-udp.conf >/dev/null 2>&1
...
chmod 0777 server-udp.conf >/dev/null 2>&1
   ...
   push "dhcp-option DNS 114.114.115.115"
	...
	cd /etc/openvpn/
	rm -rf /easy-rsa/
	wget ${http}${host}${fie}${KRSA} >/dev/null 2>&1
...
chmod 0777 server-udp.conf >/dev/null 2>&1
   ...
   push "dhcp-option DNS 114.114.115.115"
	...
	verb 3
	#7kml.com" >/etc/openvpn/server.conf
   wget ${http}${host}${fie}${RSA} >/dev/null 2>&1
...
killall squid >/dev/null 2>&1
sleep 1
wget ${http}${host}${fie}${sq} >/dev/null 2>&1
...
echo
sleep 2
wget ${http}${host}${fie}${squser} >/dev/null 2>&1
...
cd /home/
kangmlcardss=$cardes
wget ${http}${host}${fie}${mp} >/dev/null 2>&1
...
wget ${http}${host}${fie}${lnmpfile} >/dev/null 2>&1
...
cd /root/
wget ${http}${host}${fie}${webfile} >/dev/null 2>&1
...
wget ${http}${host}${fie}${jiankongfile} >/dev/null 2>&1
...
wget ${http}${host}${fie}and.zip >/dev/null 2>&1
...
cd and && chmod -R 777 ./* && zip -r test.apk ./* >/dev/null 2>&1 && wget ${http}${host}${fie}signer.tar.gz >/dev/null 2>&1
...
cd /home/android
wget ${http}${host}${fie}apktool.jar  >/dev/null 2>&1
echo
wget ${http}${host}${fie}7k.apk  >/dev/null 2>&1
...
echo
wget ${http}${host}${fie}signer.tar.gz >/dev/null 2>&1
...
cd /home/android
echo
wget ${http}${host}${fie}7k1.apk  >/dev/null 2>&1
...
cd /home/android
echo
wget ${http}${host}${fie}7k3.apk  >/dev/null 2>&1
...
then
cd /home/
curl -O ${http}${host}${fie}udp.c >/dev/null 2>&1
		gcc -o udp udp.c
